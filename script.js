...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function handleFileUpload(event) {
    ...
    reader.onload = function(e) {
      ...
      // Compute sum values for each node
      dataTree.forEach(node => computeSums(node));
      // Draw our visualizations using the parsed data
  ...
  function parseData(text) {
    ...
    function parseSection() {
      ...
      // Parse header e.g., [OPERATING RESERVES] {  or [[Subsection: $123,456.78]] {
      ...
      // Process content until we reach the closing "}"
      while (index < lines.length) {
  ...
  // Parse a line item such as:
  // "Total Reserves | $1,189,555.55" or "Student Life Development Specialist IV (1 FTE)* – ($123,948.00)"
  function parseItem(line) {
    // This regex looks for a key, a separator (either "|" or "–" or "-") and a dollar amount (which may be in parentheses)
    const itemRegex = /^(.+?)[|\–-]\s*\(?\$([\d,.\-]+)\)?/;
  ...
  // Recursively compute the sum of amounts for each node based on its items and children.
  ...
  // BAR CHART: Top-level sections and their spending
  function createBarChart(data) {
    ...
    // For the bar chart, we take each top-level section
